...
public class Producer extends Thread {
    ...
    public void run() 
    {
        ...
        while(messageNumber < 200)
        {
        	if(isAsync == false)
        	{
	        	String messageString = "Message_" + messageNumber + "Synchronous";
	            try 
	            {
	                producer.send(new ProducerRecord<>(topic,messageNumber, messageString)).get();
	                System.out.println("Sent message: (" + messageNumber + ", " + messageString + ")");
	                System.out.flush();
	                Thread.sleep(1000);
	            } 
        ...
        //This line is only added for debugging
        try {
